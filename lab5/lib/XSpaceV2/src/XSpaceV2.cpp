...
void XSpaceV2Board::init(int freq, double resolution){
    pinMode(nSLEEP,OUTPUT); //Este es el original.
...
void XSpaceV2Board::init_encoder(double resolution){
...
void XSpaceV2Board::init_DRV8837(int freq){
    pinMode(nSLEEP,OUTPUT); //no se si esto es necesario
...
void XSpaceV2Board::init_BTS7960(int R_ENx, int L_ENx, int RPWMx, int LPWMx, int freq, double vm){
    vm_BTS7960 = vm;
    ...
    digitalWrite(R_EN,HIGH);    //Habilitar ambos lados del puente
    digitalWrite(L_EN,HIGH);

    ledcAttachPin(RPWM, 1); //Salida RPWM es PWM y conectada al canal 1
    ledcAttachPin(LPWM, 2);
...
void XSpaceV2Board::BTS7960_Sleep(){
    digitalWrite(R_EN,LOW);
    digitalWrite(L_EN,LOW);
}

void XSpaceV2Board::BTS7960_Wake(){
...
void XSpaceV2Board::BTS7960_Voltage(double vp){
    
    if(vp>vm_BTS7960) vp = vm_BTS7960;
    if(vp<-vm_BTS7960) vp = -vm_BTS7960;
    int Duty = (int) ( (abs(vp)/vm_BTS7960) * 1024.0);

    //Definir sentido de giro. Intercambio para variar positivo a negativo. O puedo intercambiar la conexion de canales del encoder.
    if(vp<0){
        ledcWrite(1, Duty);
        ledcWrite(2, 0);
    }else{
...
double XSpaceV2Board::GetEncoderSpeed(int modo){
    ...
    case DEGREES_PER_SECOND:
        ...
        if(abs(vel-vel_ant)>200)vel=vel_ant;
        break;

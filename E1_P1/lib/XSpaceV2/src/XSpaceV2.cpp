...
void XSpaceV2Board::init(int freq, double resolution){
    ...
    ledcAttachPin(IN1, 1);
    ledcAttachPin(IN2, 2);
    ledcAttachPin(VALVULA_PIN, 3); // Asigna el canal 3 de PWM

    ledcSetup(1, freq, 10);
    ledcSetup(2, freq, 10);
    ledcSetup(3, freq, 10); // Frecuencia y resolución del canal 3
...
double XSpaceV2Board::flujo() {
    return 50;
}

void XSpaceV2Board::Valvula(double pc) {
    // Acota la salida caso de saturación
    if(pc>100) pc = 100;
    if(pc<0) pc = 0;

    int Duty = (int) (pc/100*1024);
    ledcWrite(3, Duty);
}
